...
| 9 | 権限 | string | 255 |  |  | o | general | admin: 管理者<br />general: 一般[^1] |
...
| 6 | 目安納期日数 | integer | 2,147,483,647|  |  | o | 3 | 営業日の数字を記載。土日祝日は含みません |
...
| 4 | 目安納期日数 | integer | 2,147,483,647|  |  |   | 3 | 営業日の数字を記載。土日祝日は含みません |
| 5 | 開始日時 | datetime | | o | | |   | |
| 6 | 終了日時 | datetime | |   | | |   | |

#### 単価の適用ルール

- 単価が適用されるかどうかは、有効化フラグではなく開始日時・終了日時で判定されます。
- 終了日時を過ぎた単価は、有効化フラグがONのままでも適用されません。
- 開始日時・終了日時の条件に合っている単価が複数ある場合は、開始日時が新しい単価が適用されます。
- 一定期間だけ特別価格を適用したい場合は、その期間だけ別単価を登録し、その前後の期間に通常単価を登録してください。
- 同じ型式（品番）かつ同じ小売コードで、同じ開始日時の単価を複数登録することはできません。
